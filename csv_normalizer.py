...
def main(source, target):
    ...
    for file_name, in_file_name_path, out_file_name_and_path in zip(files_name, in_files_name_and_path, out_files_name_and_path):
        step_1_count, step_2_lines, normalize_time = normalizer(file_name, in_file_name_path, out_file_name_and_path)
        log_creation(step_1_count, step_2_lines, file_name, target, normalize_time)

    print('Total Elapsed Time: {:.2f}'.format(time.time() - start))


def log_creation(step_1_count, step_2_lines, file_name, target, timer):
    ...
    with open(target + time_tag + '_' + log_file_name + '.txt', 'w') as log_file:
        ...
        log_file.write('Step 2 - Number of line delimiters " corrected: {}\n'.format(int(len(step_2_lines) / 2)))
        log_file.write('Normalizing time: {:.3f}\n'.format(timer))
    log_file.close()


def multireplace(string, replacements, step_2_lines):
    ...
    for x in sel_iterator:
        if string[x.end() + number_of_corrections] == ';':  # Campos vazios
            ...
            step_2_lines.append(string[1:-1])
...
def normalizer(file_name, in_file_name, out_file_name):
    ...
    run_once, step_1_count, step_2_lines = 0, 0, []
        ...
        with open(out_file_name, 'w') as out_file:
            ...
            for line in in_file:
                ...
                if current_delimiter_count == delimiter_count:
                    ...
                    total_line, step_2_lines = multireplace(total_line, replacements, step_2_lines)
    ...
    return step_1_count, step_2_lines, time_to_normalize
